...
def create_or_update_run(run_id: str, fields: dict[str, Any]) -> None:
    get_ctx().services.supabase.create_or_update_run(run_id, fields)


def append_run_event(run_id: str, event: dict[str, Any], seq: int) -> None:
    get_ctx().services.supabase.append_run_event(run_id, event, seq)
...
def append_run_message(
    ...
    *,
    ...
    get_ctx().services.supabase.append_run_message(
...
def finalize_run(
    ...
    *,
    ...
    get_ctx().services.supabase.finalize_run(
        run_id,
        status=status,
...
def list_runs(limit: int = 50, offset: int = 0, status: str | None = None) -> list[dict[str, Any]]:
    return get_ctx().services.supabase.list_runs(limit=limit, offset=offset, status=status)


def get_run(run_id: str) -> dict[str, Any] | None:
    return get_ctx().services.supabase.get_run(run_id)


def get_run_history(run_id: str) -> dict[str, Any]:
    return get_ctx().services.supabase.get_run_history(run_id)
