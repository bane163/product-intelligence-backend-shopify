...
async def save_product_draft(
    ...
    saved = save_product_draft_execute(supabase=ctx.supabase, draft_id=draft_id, run_id=run_id, import_mode=import_mode, draft_name=draft_name, input_file_id=input_file_id, input_filename=input_filename, output_file_id=output_file_id, output_filename=output_filename, products=products)
    return {
...
async def list_product_drafts(
    ...
    drafts = list_drafts_execute(supabase=ctx.supabase, limit=limit, offset=offset, search=search, sort_by=sort_by, sort_dir=sort_dir)
...
async def get_product_draft(draft_id: str, ctx: AppContext = Depends(get_ctx)) -> dict[str, Any]:
    ...
    draft = get_draft_execute(supabase=ctx.supabase, draft_id=draft_id)
    if not draft:
...
async def create_product_draft_resume_file(
    ...
    try:
        return create_resume_execute(supabase=ctx.supabase, draft_id=draft_id)
    except LookupError:
        raise HTTPException(status_code=404, detail="Draft not found")
    except ValueError:
...
async def delete_product_draft(draft_id: str, ctx: AppContext = Depends(get_ctx)) -> dict[str, str]:
    ...
    if not delete_draft_execute(supabase=ctx.supabase, draft_id=draft_id):
        raise HTTPException(status_code=404, detail="Draft not found")
