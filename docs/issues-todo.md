...
- **Details:** Updated metafields lookup to GraphQL `keys` + `nodes` contract and added fallback query without `keys` when a shop/API surface rejects the `keys` argument. Added regression coverage for Shopify client fallback and apply-suggestion metafields flow so apply-fixes does not fail on metafields retrieval drift.
